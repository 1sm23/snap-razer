...
const enMessages = {
  ...
  "connection.conflictStepPermission": "Clear this site's HID permission in the browser, reconnect, then try the other same-named Razer entry if one appears.",
  ...
  "buttonMap.description": "Prepare a per-device mapping profile for the buttons most Razer mice expose.",
  "buttonMap.firmwareNotice":
    "Saved locally for now. Firmware write support will be enabled only after this device's button report is verified.",
...
const zhCNMessages: Record<MessageKey, string> = {
  ...
  "connection.conflictStepPermission": "清除浏览器里此站点的 HID 权限，重新连接；如果出现另一个同名雷蛇条目，也试一下那个。",
  "connection.nativeDriverConflict":
  ...
  "buttonMap.description": "为大多数雷蛇鼠标会暴露的按键准备一份设备配置。",
  "buttonMap.firmwareNotice": "当前先保存在本地；确认此设备的按键报告后，才会启用写入固件。",
  "buttonMap.reset": "重置",
